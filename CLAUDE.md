...
- **Name display** (2026-09-26): site-level display uses "Matthew Fails" (`params.owner.short_name`: navbar brand, homepage heading, footer name, copyright, browser tab titles). Citations, author lists, and scholarly metadata (citation_* tags, JSON-LD) keep "Matthew D. Fails" (`params.owner.name`). Don't change `owner.name`; it's how templates recognize the owner in `authors:` lists.
